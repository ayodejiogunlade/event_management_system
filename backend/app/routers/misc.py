...
def run_planner(query: PlannerQuery, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def run_matching(query: MatchQuery, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return match_vendors(db, query)


# ── Meta (categories + pricing models from DB) ────────────────────────────────
...
def mark_all_read(db: Session = Depends(get_db), cu=Depends(get_current_user)):
    db.query(models.Notification)\
      .filter(models.Notification.user_id == cu.id, models.Notification.is_read == False)\
      .update({"is_read": True})
    db.commit()
...
def mark_read(notif_id: int, db: Session = Depends(get_db), cu=Depends(get_current_user)):
    n = db.query(models.Notification)\
          .filter(models.Notification.id == notif_id, models.Notification.user_id == cu.id).first()
    if n:
...
def get_stats(db: Session = Depends(get_db), _=Depends(admin_only)):
    return {
        ...
        "total_services":     db.query(models.VendorService).filter(models.VendorService.is_active == True).count(),
    }
...
def update_user(uid: int, data: AdminUserUpdate, db: Session = Depends(get_db), _=Depends(admin_only)):
...
def verify_vendor(vid: int, data: VendorVerify, db: Session = Depends(get_db), _=Depends(admin_only)):
    v = db.query(models.Vendor).filter(models.Vendor.id == vid).first()
    if not v: raise HTTPException(404)
    v.is_verified = data.is_verified; db.commit()
...
def update_service_limit(vid: int, data: VendorServiceLimitUpdate,
    ...
    v = db.query(models.Vendor).filter(models.Vendor.id == vid).first()
    if not v: raise HTTPException(404)
    v.service_limit = data.service_limit; db.commit()
...
def list_categories(db: Session = Depends(get_db), _=Depends(admin_only)):
    return db.query(models.ServiceCategoryDef).order_by(models.ServiceCategoryDef.sort_order).all()

@admin_router.post("/categories", response_model=CategoryDefOut, status_code=201)
def create_category(data: CategoryDefCreate, db: Session = Depends(get_db), _=Depends(admin_only)):
    if db.query(models.ServiceCategoryDef).filter(models.ServiceCategoryDef.key == data.key).first():
...
def update_category(cid: int, data: CategoryDefUpdate, db: Session = Depends(get_db), _=Depends(admin_only)):
    c = db.query(models.ServiceCategoryDef).filter(models.ServiceCategoryDef.id == cid).first()
    if not c: raise HTTPException(404)
    for k, v in data.model_dump(exclude_none=True).items():
        setattr(c, k, v)
    db.commit(); db.refresh(c); return c

# ── Admin: Manage Pricing Models ──────────────────────────────────────────────
...
def create_pricing_model(data: PricingModelDefCreate, db: Session = Depends(get_db), _=Depends(admin_only)):
    if db.query(models.PricingModelDef).filter(models.PricingModelDef.key == data.key).first():
