...
function EventModal({ event, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    budget: event?.budget || '', location_address: event?.location_address || '',
    location_lat: event?.location_lat || '', location_lng: event?.location_lng || '',
    required_services: event?.required_services || '', description: event?.description || '',
  });
        ...
        <div className="modal-body">
          <form onSubmit={submit}>
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select className="form-control" value={form.event_type} onChange={f('event_type')}>
                  {['Conference','Wedding','Exhibition','Corporate','Party','Concert','Workshop','Other'].map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
            </div>
            <div className="form-row">
              ...
              <div className="form-group">
                <label>Budget (₦)</label>
                <input className="form-control" type="number" value={form.budget} onChange={f('budget')} placeholder="500000" />
              </div>
            </div>
            <div className="form-group">
              <label>Venue Address</label>
              <input className="form-control" value={form.location_address} onChange={f('location_address')} placeholder="123 Adeola Odeku St, Victoria Island, Lagos" />
            </div>
            <div className="form-row">
              <div className="form-group">
            ...
            <div className="form-group">
              ...
              <input className="form-control" value={form.required_services} onChange={f('required_services')} placeholder="Catering, Decoration, Photography" />
            </div>
            <div className="form-group">
              <label>Description</label>
              <textarea className="form-control" value={form.description} onChange={f('description')} placeholder="Brief description of the event…" />
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button className="btn btn-secondary" type="button" onClick={onClose}>Cancel</button>
              <button className="btn btn-primary" type="submit" disabled={loading}>{loading ? 'Saving…' : editing ? 'Update Event' : 'Create Event'}</button>
            </div>
          </form>
        </div>
      </div>
...
export default function Events() {
  ...
  const cancelEvent = async id => {
    if (!window.confirm('Cancel this event? All bookings will be cancelled.')) return;
    try { await eventsAPI.cancel(id); toast.success('Event cancelled'); load(); }
    catch (err) { toast.error(err.response?.data?.detail || 'Failed'); }
  };

  const openEdit = ev => { setEditing(ev); setShowModal(true); };
  ...
  return (
    <div className="app-shell">
      ...
      <div className="main-content">
        ...
        <div className="page">
          <div className="flex justify-between items-center mb-3">
            <div><p className="page-title">Events</p><p className="page-subtitle">Plan and manage all your events</p></div>
            <button className="btn btn-primary" onClick={openNew}><Plus size={16} /> New Event</button>
          </div>

          {loading ? <div className="loading"><div className="spinner" /></div> :
            events.length === 0 ? (
              <div className="card"><div className="card-body"><div className="empty-state"><div className="icon">📅</div><p>No events yet. Create your first one!</p><br/><button className="btn btn-primary" onClick={openNew}>+ Create Event</button></div></div></div>
            ) : (
              <div className="card">
                <div className="table-wrap">
                  <table>
                    <thead>
                      <tr><th>Event Name</th><th>Type</th><th>Date</th><th>Venue</th><th>Budget</th><th>Status</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                      {events.map(ev => (
                        <tr key={ev.id}>
                          ...
                          <td>
                            <div className="flex gap-2">
                              {ev.status !== 'cancelled' && <>
                                <button className="btn btn-ghost btn-sm" onClick={() => openEdit(ev)} title="Edit"><Edit2 size={14} /></button>
                                <button className="btn btn-ghost btn-sm text-danger" onClick={() => cancelEvent(ev.id)} title="Cancel"><Trash2 size={14} /></button>
                              </>}
                            </div>
                          </td>
                        </tr>
      ...
      {showModal && <EventModal event={editing} onClose={() => setShowModal(false)} onSaved={onSaved} />}
    </div>
  );
}
