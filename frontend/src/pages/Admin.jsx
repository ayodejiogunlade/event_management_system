...
function VendorDrawer({ vendor, onClose, onUpdated }) {
  ...
  const setLimit = async () => {
    setUpdating(true)
    try { await adminAPI.setServiceLimit(vendor.id, parseInt(newLimit)); toast.success('Service limit updated!'); onUpdated() }
  ...
  const toggleVerify = async () => {
    setUpdating(true)
    try { await adminAPI.verifyVendor(vendor.id, !vendor.is_verified); toast.success(vendor.is_verified ? 'Verification revoked' : 'Vendor verified!'); onUpdated() }
    catch { toast.error('Failed') }
    setUpdating(false)
  }

  const priceSummary = svc => {
    if (svc.pricing_model_key === 'fixed_fee'  && svc.fixed_price)     return `₦${Number(svc.fixed_price).toLocaleString()} fixed`
    if (svc.pricing_model_key === 'per_head'   && svc.price_per_head)  return `₦${Number(svc.price_per_head).toLocaleString()}/guest`
    if (svc.pricing_model_key === 'percentage' && svc.percentage_rate) return `${svc.percentage_rate}% of budget`
    if (svc.pricing_model_key === 'hourly'     && svc.hourly_rate)     return `₦${Number(svc.hourly_rate).toLocaleString()}/hr`
    return '—'
  }
      ...
      <div className="modal" style={{ maxWidth: 700, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div className="modal-body">
          <div className="flex gap-2 mb-3" style={{ flexWrap: 'wrap', marginBottom: 16 }}>
            {vendor.is_verified ? <span className="badge badge-green">✅ Verified</span> : <span className="badge badge-amber">⏳ Pending Verification</span>}
            <span className={`badge ${vendor.availability_status ? 'badge-green' : 'badge-gray'}`}>{vendor.availability_status ? '● Available' : '○ Unavailable'}</span>
            <span className="badge badge-blue">⭐ {vendor.rating?.toFixed(1)} ({vendor.rating_count} reviews)</span>
            <span className="badge badge-purple">Limit: {vendor.service_limit === -1 ? 'Unlimited' : vendor.service_limit}</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {[
              ['Owner', vendor.owner_name], ['Email', vendor.owner_email],
          ...
          <div style={{ marginBottom: 16 }}>
            <p className="section-title" style={{ marginBottom: 10 }}>Services ({vendor.services?.length || 0})</p>
            {!vendor.services?.length ? (
              <div className="empty-state" style={{ padding: '20px 0' }}><p>No services listed yet</p></div>
            ) : (
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          ...
                          <span className="badge badge-blue"   style={{ fontSize: 10 }}>{svc.pricing_model_key}</span>
          ...
          <div style={{ background: 'var(--gray-50)', border: '1px solid var(--gray-200)', borderRadius: 10, padding: 16 }}>
            ...
            <div className="flex gap-3" style={{ flexWrap: 'wrap' }}>
              <button className={`btn ${vendor.is_verified ? 'btn-danger' : 'btn-success'}`} onClick={toggleVerify} disabled={updating}>
                {vendor.is_verified ? '❌ Revoke Verification' : '✅ Verify Vendor'}
              </button>
              <div className="flex gap-2 items-center">
                <select className="form-control" style={{ width: 220, margin: 0 }} value={newLimit} onChange={e => setNewLimit(e.target.value)}>
                  {LIMIT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <button className="btn btn-primary" onClick={setLimit} disabled={updating}>{updating ? '…' : 'Set Limit'}</button>
              </div>
            </div>
          </div>
        </div>
...
export function AdminUsers() {
  ...
  const toggle = async (id, active) => {
    try { await adminAPI.updateUser(id, { is_active: !active }); toast.success(!active ? 'User activated' : 'User suspended'); load() }
    catch { toast.error('Failed') }
  }
        ...
        <div className="page">
          ...
          <div className="card">
            <div className="card-header flex justify-between items-center">
              ...
              <input className="form-control" style={{ width: 260, margin: 0 }} placeholder="Search by name or email…" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
                  ...
                  <tbody>
                    {filtered.map(u => (
                      <tr key={u.id}>
                        ...
                        <td><button className={`btn btn-sm ${u.is_active ? 'btn-danger' : 'btn-success'}`} onClick={() => toggle(u.id, u.is_active)}>{u.is_active ? 'Suspend' : 'Activate'}</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
export function AdminVendors() {
  ...
  const onUpdated = () => {
    load()
    if (selected) adminAPI.getVendor(selected.id).then(r => setSelected(r.data)).catch(() => setSelected(null))
  }

  const filtered = filter === 'all' ? vendors : filter === 'pending' ? vendors.filter(v => !v.is_verified) : vendors.filter(v => v.is_verified)
    ...
    <div className="app-shell">
      ...
      <div className="main-content">
        ...
        <div className="page">
          ...
          <p className="page-subtitle">Verify vendors, view services, and manage service limits</p>
          {pending > 0 && (
          ...
          <div className="card">
            {loading ? <div className="loading"><div className="spinner" /></div> : (
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Business</th><th>Owner</th><th>Services</th><th>Rating</th><th>Svc Limit</th><th>Status</th><th>Actions</th></tr></thead>
                  <tbody>
                    {filtered.map(v => (
                      <tr key={v.id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {v.services?.slice(0, 2).map(s => <span key={s.id} className="badge badge-purple" style={{ fontSize: 10 }}>{s.category_key}</span>)}
                        ...
                        <td><span className={`badge ${v.service_limit === -1 ? 'badge-green' : 'badge-blue'}`}>{v.service_limit === -1 ? '∞' : v.service_limit}</span></td>
                        <td>{v.is_verified ? <span className="badge badge-green">✅ Verified</span> : <span className="badge badge-amber">⏳ Pending</span>}</td>
                        <td><button className="btn btn-secondary btn-sm" onClick={() => setSelected(v)}><Eye size={13} /> View</button></td>
                      </tr>
                    ))}
                    {filtered.length === 0 && <tr><td colSpan={7}><div className="empty-state" style={{ padding: '40px 0' }}><p>No vendors found</p></div></td></tr>}
                  </tbody>
                </table>
              </div>
            )}
        ...
        <div className="page">
          ...
          <div className="card">
            <div className="card-header flex justify-between items-center">
              ...
              <input className="form-control" style={{ width: 260, margin: 0 }} placeholder="Search by name or organizer…" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
            {loading ? <div className="loading"><div className="spinner" /></div> : (
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Event</th><th>Type</th><th>Organizer</th><th>Date</th><th>Budget</th><th>Bookings</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    {filtered.map(ev => (
                      <tr key={ev.id}>
                        ...
                        <td><button className="btn btn-ghost btn-sm" onClick={() => setSelected(ev)}><Eye size={13} /></button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selected && (
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setSelected(null)}>
          <div className="modal" style={{ maxWidth: 620 }}>
            <div className="modal-header"><h2>📅 {selected.name}</h2><button className="btn btn-ghost btn-sm" onClick={() => setSelected(null)}>✕</button></div>
            <div className="modal-body">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ['Type', selected.event_type], ['Date', selected.event_date ? format(new Date(selected.event_date), 'dd MMM yyyy HH:mm') : '—'],
                  ['Organizer', selected.organizer_name], ['Contact', selected.organizer_email],
                  ['Budget', selected.budget ? `₦${Number(selected.budget).toLocaleString()}` : '—'], ['Status', selected.status],
                  ['Venue', selected.location_address || '—'], ['Bookings', selected.booking_count],
                ].map(([k, v]) => (
                  <div key={k} style={{ background: 'var(--gray-50)', borderRadius: 8, padding: '10px 14px' }}>
                    <div style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>{k}</div>
            ...
            {['all','pending','confirmed','declined','cancelled','completed'].map(s => (
              <button key={s} className={`btn btn-sm ${filter === s ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setFilter(s)} style={{ textTransform: 'capitalize' }}>
                {s}{s !== 'all' && bookings.filter(b => b.status === s).length > 0 &&
                  <span style={{ marginLeft: 4, background: 'rgba(255,255,255,.3)', borderRadius: 99, padding: '0 5px' }}>{bookings.filter(b => b.status === s).length}</span>}
              </button>
            ))}
          </div>
          <div className="card">
            {loading ? <div className="loading"><div className="spinner" /></div> : (
              <div className="table-wrap">
                <table>
                  <thead><tr><th>#</th><th>Event</th><th>Organizer</th><th>Vendor</th><th>Service</th><th>Price</th><th>Status</th><th></th></tr></thead>
                  <tbody>
                    {filtered.map(b => (
                      <tr key={b.id}>
                        ...
                        <td className="fw-600" style={{ color: 'var(--green)' }}>{b.agreed_price ? `₦${Number(b.agreed_price).toLocaleString()}` : '—'}</td>
                        <td>{statusBadge(b.status)}</td>
                        <td><button className="btn btn-ghost btn-sm" onClick={() => setSelected(b)}><Eye size={13} /></button></td>
                      </tr>
                    ))}
                    {filtered.length === 0 && <tr><td colSpan={8}><div className="empty-state" style={{ padding: '40px 0' }}><p>No bookings found</p></div></td></tr>}
                  </tbody>
                </table>
              </div>
            )}
      ...
      {selected && (
        <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && setSelected(null)}>
          <div className="modal" style={{ maxWidth: 580 }}>
            <div className="modal-header"><h2>📋 Booking #{selected.id}</h2><button className="btn btn-ghost btn-sm" onClick={() => setSelected(null)}>✕</button></div>
            <div className="modal-body">
              <div style={{ marginBottom: 12 }}>{statusBadge(selected.status)}</div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                {[
                  ['Event', selected.event_name], ['Event Date', selected.event_date ? format(new Date(selected.event_date), 'dd MMM yyyy') : '—'],
                  ['Organizer', selected.organizer_name], ['Vendor', selected.vendor_name],
                  ['Service', selected.vendor_service || '—'], ['Category', selected.service_category || '—'],
                  ['Guest Count', selected.guest_count || '—'], ['Agreed Price', selected.agreed_price ? `₦${Number(selected.agreed_price).toLocaleString()}` : '—'],
                ].map(([k, v]) => (
                  <div key={k} style={{ background: 'var(--gray-50)', borderRadius: 8, padding: '10px 14px' }}>
                    <div style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>{k}</div>
                    <div style={{ fontSize: 13, color: 'var(--gray-800)' }}>{String(v ?? '—')}</div>
              ...
              {selected.service_details && (
                <div style={{ marginTop: 14, background: 'var(--gray-50)', borderRadius: 8, padding: '12px 14px' }}>
                  <div style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 4 }}>Service Details</div>
                  <p style={{ fontSize: 13 }}>{selected.service_details}</p>
                </div>
              )}
            </div>
          </div>
