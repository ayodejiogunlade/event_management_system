        ...
        <div className="modal-body">
          <form onSubmit={save}>
            ...
            <div className="form-group">
              ...
              <input className="form-control" required value={form.label} onChange={f('label')} placeholder="e.g. Live Band" />
            </div>
            <div className="form-group">
      ...
      <div className="modal" style={{ maxWidth: 480 }}>
        ...
        <div className="modal-body">
          <form onSubmit={save}>
            ...
            <div className="form-group">
              <label>Label *</label>
              <input className="form-control" required value={form.label} onChange={f('label')} placeholder="e.g. Per Day Rate" />
            </div>
            <div className="form-group">
              <label>Description</label>
              <textarea className="form-control" rows={2} value={form.description} onChange={f('description')}
...
export default function AdminSettings() {
  ...
  return (
    <div className="app-shell">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <p className="text-sm text-muted" style={{ marginBottom: 14 }}>
                    Sets how many services a newly registered vendor can add by default. Individual limits can be upgraded per-vendor in the Vendor Management page.
                  </p>
                  <div className="flex gap-3 items-center" style={{ flexWrap: 'wrap' }}>
                    {LIMIT_OPTIONS.map(o => (
                      <button key={o.value} className={`btn ${defaultLimit === o.value ? 'btn-primary' : 'btn-secondary'}`}
              ...
              <div className="card">
                ...
                <div className="table-wrap">
                  <table>
                    <thead><tr><th>Icon</th><th>Key</th><th>Label</th><th>Order</th><th>Status</th><th>Action</th></tr></thead>
                    <tbody>
                      {categories.map(c => (
                        <tr key={c.id}>
                          ...
                          <td><button className="btn btn-ghost btn-sm" onClick={() => setCatModal(c)}><Edit2 size={13} /></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {pricingModels.map(p => (
                        <tr key={p.id}>
                          ...
                          <td><button className="btn btn-ghost btn-sm" onClick={() => setPmModal(p)}><Edit2 size={13} /></button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {catModal !== null && (
        <CategoryModal key={catModal.id ?? 'new-cat'} cat={catModal.id ? catModal : null}
          onClose={() => setCatModal(null)} onSaved={() => { setCatModal(null); load() }} />
      )}
      {pmModal !== null && (
        <PricingModal key={pmModal.id ?? 'new-pm'} model={pmModal.id ? pmModal : null}
          onClose={() => setPmModal(null)} onSaved={() => { setPmModal(null); load() }} />
      )}
    </div>
  )
}
